...
app.get('/products', async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT id, name, price, category, description FROM products')
        res.send(rows)
    } catch (err) {
...
app.put('/products/:id', async (req, res) => {
    try {
        const { id } = req.params
        const { name, price, category, description } = req.body
        const sql = 'UPDATE products SET name = ?, price = ?, category = ?, description = ? WHERE id = ?'
